...
void bullet::fire(float x, float y, float angle, float speed)
{
	...
	//총알 구조체 선언
	tagBullet bullet;
	//멤셋, 제로메모리를 이용해서 구조체 변수들을 일괄 0으로 초기화
...
{
	_viBullet = _vBullet.begin();
	for (; _viBullet != _vBullet.end();)//while(true)
	{
...
//====================================================================
//			## missile ## (missile[0] => 배열처럼 장전해서 총알발사)
//====================================================================
...
void missile::render(void)
{
	...
	for (_viBullet; _viBullet != _vBullet.end(); ++_viBullet)
	{
		...
		_viBullet->count++;
		if (_viBullet->count % 2 == 0)//총알애니메이션 속도
		{
			_viBullet->bulletImage->setFrmaeX(_viBullet->bulletImage->getFrameX() + 1);
			if (_viBullet->bulletImage->getFrameX() >= _viBullet->bulletImage->getMaxFrameX())
			{
				_viBullet->bulletImage->setFrmaeX(0);
			}
			_viBullet->count = 0;
		}
	}
